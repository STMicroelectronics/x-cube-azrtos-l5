...
void CDC_Init_FS(void *cdc_acm)
{
  ...
  /* Get uart3 baudrate */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_baudrate = hlpuart1.Init.BaudRate;

  /*set the data type : only 8bits and 9bits are supported */
  switch (hlpuart1.Init.WordLength)
  {
  ...
  /* Get uart3 Parity */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_parity = hlpuart1.Init.Parity;

  /* Get uart3 StopBits */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_stop_bit = hlpuart1.Init.StopBits;
    ...
    if (device->ux_slave_device_state == UX_DEVICE_CONFIGURED)
    {
      ...
      if (ux_status == UX_SUCCESS)
      {
        ...
        if (actual_length != 0)
        {
          ...
          if (HAL_UART_Transmit_DMA(&hlpuart1, (uint8_t *)UserRxBufferFS,
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (HAL_UART_Receive_IT(&hlpuart1, (uint8_t *)UserTxBufferFS + UserTxBufPtrIn, 1) != HAL_OK)
  {
    /* Transfer error in reception process */
    Error_Handler();
  }
                                ...
                                *CDC_VCP_LineCoding)
{
  ...
  switch (CDC_VCP_LineCoding->ux_slave_class_cdc_acm_parameter_data_bit)
  {
    ...
    case 0x08:
    {
      if (hlpuart1.Init.Parity == UART_PARITY_NONE)
      {
        hlpuart1.Init.WordLength = UART_WORDLENGTH_8B;
      }
      else
  ...
  hlpuart1.Init.BaudRate = CDC_VCP_LineCoding->ux_slave_class_cdc_acm_parameter_baudrate;
  ...
  HAL_UART_Receive_IT(&hlpuart1, (uint8_t *)(UserTxBufferFS + UserTxBufPtrIn), 1);
}
/* USER CODE END 0 */
