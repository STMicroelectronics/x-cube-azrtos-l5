...
__weak int32_t BSP_USBPD_PWR_Init(uint32_t Instance)
{
  ...
  LL_GPIO_SetPinMode(TCPP01_DB_GPIO_Port, TCPP01_DB_Pin, LL_GPIO_MODE_OUTPUT);
  LL_GPIO_SetPinSpeed(TCPP01_DB_GPIO_Port, TCPP01_DB_Pin, LL_GPIO_SPEED_FREQ_HIGH);
  LL_GPIO_SetPinOutputType(TCPP01_DB_GPIO_Port, TCPP01_DB_Pin, LL_GPIO_OUTPUT_PUSHPULL);
  LL_GPIO_SetPinPull(TCPP01_DB_GPIO_Port, TCPP01_DB_Pin, LL_GPIO_PULL_UP);
  LL_GPIO_SetOutputPin(TCPP01_DB_GPIO_Port, TCPP01_DB_Pin);
...
__weak int32_t BSP_USBPD_PWR_VBUSGetVoltage(uint32_t Instance, uint32_t *pVoltage)
{
  ...
  else
  {
    ...
    voltage = (uint32_t) __LL_ADC_CALC_DATA_TO_VOLTAGE(VDDA_APPLI, (uint32_t) LL_ADC_REG_ReadConversionData12(hadc1.Instance), LL_ADC_RESOLUTION_12B); /* mV */

    /* STM32L562E_DISCOVERY board is used */
    /* (Divider R17/R13 (49.9K/330K) for VSENSE */
    voltage *= 7613u;
    voltage /= 1000u;
